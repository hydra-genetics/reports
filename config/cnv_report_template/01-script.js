...
const plotChromosomeView = function(plotData) {
    ...
    const yAxis = g => g
        .call(d3.axisLeft(yScale).ticks(8).tickFormat((y, i) => i % 2 == 0 ? y : ""));
    ...
    const lrGrid = lrArea.append("g")
        .attr("class", "grid");
    const vafGrid = vafArea.append("g")
        .attr("class", "grid");
            ...
            .join(
                enter => {
                    ...
                    annotation_group.append("rect")
                        ...
                        .attr("x", d => {
                            let [labelWidth, labelHeight] = getTextDimensions(d.name, "0.8rem");
                    ...
                    update.selectAll(".annotation-marker")
                        .attr("fill-opacity", 0.05)
                        .call(update => update.transition()
                            .attr("x", d => xScale(d.start))
                            .attr("width", d => xScale(d.end) - xScale(d.start))
    ...
    svg.append("g")
        ...
        .call(d3.drag()
            ...
            .on("end", (e) => {
                ...
                zoomToRegion(data.chromosome, xScale.invert(xMin), xScale.invert(xMax), 0);
            }))
        .on("click", e => {
...
const plotGenomeView = () => {
    ...
    const lrGrid = lrPanels.append("g")
        .attr("class", "grid")
        .attr("data-index", (d, i) => i);
    const vafGrid = vafPanels.append("g")
        .attr("class", "grid")
        .attr("data-index", (d, i) => i);
    ...
    const plotRegions = () => {
        ratioPanels
            ...
            .data(d => d.callers[getActiveCaller()].ratios.filter((r, i) => i % 5 === 0), d => d.start)
            .join(
                enter => enter
                    .append("circle")
                    .attr("class", "point")
    ...
    const plotSegments = () => {
        segmentPanels
            ...
            .data(d => d.callers[getActiveCaller()].segments.filter(s => s.end - s.start > totalLength/width), d => [d.start, d.end, d.log2])
            .join(
                enter => enter
                    .append("path")
                    .attr("class", "segment")
...
const populateTable = () => {
    ...
    const tooltip = d3.select(".container")
        ...
        .call(d => d.append("table")
            .call(t => t.append("thead")
                .append("tr")
                .selectAll("th")
                .data(["caller", "type", "cn"])
                .join("th")
    ...
    const update = () => {
        ...
        tableHeader
            .selectAll("th")
            .data(Object.keys(tableData[0]).filter(k => k !== "others"), d => d)
            .join("th")
                .text(getColumnLabel)
        ...
        tableBody.selectAll(".tooltip-trigger")
            ...
            .on("mousemove", e => {
                ...
                const maxHeight = document.documentElement.clientHeight - offset - dims.height;
                tooltip
