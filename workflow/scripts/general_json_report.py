...
def metadata(sample):
    # Still in progress
    analysis_date = 'No date'
    pipeline = dict(name='', version='', uri="")
    dict_ = dict(sample=sample, analysis_date=analysis_date, pipeline=pipeline)
    return dict_
...
def generate_json(output_files, sample):
...
if __name__ == "__main__":
    ...
    additional_metadata = snakemake.input.additional_metadata
    sample = snakemake.params.sample
    json_file = generate_json(output_files, sample)
